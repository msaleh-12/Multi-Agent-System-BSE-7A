...
def supervisor_analyze_student(
    ...
    Accepts supervisor request format and returns comprehensive analysis
    """
    # Map student ID to internal user ID
    user_id = map_student_id_to_user_id(request.student_id, db)
    
    if not user_id:
        # If student not found in our system, return a default response
        # In production, you might want to create a new user or handle differently
        raise HTTPException(
